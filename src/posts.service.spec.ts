...
describe('PostsService', () => {
  ...
  describe('.findMany', () => {
    ...
    it('should update post in posts', () => {
      const createdPost = postsService.create({ text: 'original text' });

      postsService.update(createdPost.id, { text: 'updated text' });

      const updatedPost = postsService.find(createdPost.id);
